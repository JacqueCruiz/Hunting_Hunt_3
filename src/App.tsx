...
function AnatomyPanel({ onSelect, selected }: { onSelect: (organ: Organ) => void; selected: Organ }) {
  return (
    <aside className="anatomy-panel panel">
      ...
      <div className="organ-list">
        {(Object.keys(organInfo) as Organ[]).map((organ) => {
          ...
          const isVital = organ === 'heart' || organ === 'brain'

          return (
            <button key={organ} className={`organ-button ${selected === organ ? 'selected' : ''}`} onClick={() => onSelect(organ)}>
              <span className="organ-icon" style={{ color: item.color }}>{item.icon}</span>
              <span>
                <strong>{item.label}</strong>
                <small>{isVital ? 'Clean takedown' : 'Resource harvest'}</small>
              </span>
            </button>
          )
        })}
...
function Scope({ active, selected, onFire }: { active: boolean; selected: Organ; onFire: () => void }) {
  ...
  return (
    <div className={`scope ${active ? 'scope-active' : ''}`}>
      <div className="scope-sky">
        <div className="mountain mountain-back" />
        <div className="mountain mountain-front" />
        <div className="animal-silhouette" />
      </div>

      {active && (
        <div className="reticle" style={{ transform: `translate(${sway.x}px, ${sway.y}px)` }}>
          <span />
          <i />
          <b />
        </div>
...
function TrackingScene({ onTravel }: { onTravel: () => void }) {
  return (
    <section className="tracking-panel panel">
      <div className="eyebrow">FIELD TRAIL</div>
      <h2>Blood and broken brush</h2>
...
function ProcessingScene({ onFinish }: { onFinish: (kept: InventoryItem[]) => void }) {
  ...
  const finishPart = () => {
    ...
    const nextPart = {
      id: current,
      label: organInfo[current].label,
      quality: result.quality,
      color: item.color,
    }

    if (result.kept) {
      setKept((previous) => [...previous, nextPart])
    }

    if (index < processingOrder.length - 1) {
      setIndex(index + 1)
      return
    }
  ...
  return (
    <main className="processing-view">
      ...
      <section className="panel processing-card">
        ...
        <h1>Careful hands</h1>
        <p className="muted">Follow the highlighted cut line. Quality decides what the camp can preserve.</p>

        <div className="progress"><span style={{ width: `${((index + 1) / processingOrder.length) * 100}%` }} /></div>

        <div className="current-part">
          <span className="large-organ" style={{ color: item.color }}>{item.icon}</span>
          <div>
            <div className="eyebrow">PART {index + 1} OF {processingOrder.length}</div>
            <h2>{item.label}</h2>
            <p className="muted">Make a steady, controlled cut.</p>
          </div>
        </div>
...
export default function App() {
  ...
  const [animal] = useState(starterAnimal)
  ...
  const weapon = weapons[Math.min(weaponLevel - 1, weapons.length - 1)]

  const phaseLabel = useMemo(
    () => (phase === 'hunt' ? 'PINEBREAK VALLEY' : phase === 'tracking' ? 'FOLLOW THE TRAIL' : 'HOME CAMP'),
    [phase],
  )
  ...
  const handleUpgrade = (upgradeId: string) => {
    ...
    if (!upgrade) return
    if (ownedUpgrades.includes(upgradeId)) return
    if (supplies < upgrade.cost) {
      setNotice(`You need ${upgrade.cost - supplies} more supplies for ${upgrade.name}.`)
      return
    }

    setSupplies((current) => current - upgrade.cost)
    setOwnedUpgrades((current) => [...current, upgradeId])
    setNotice(`${upgrade.name} installed. ${upgrade.effect}.`)
  }

  const handleFire = () => {
    if (!aiming) {
      setAiming(true)
      setNotice(`Aiming at ${selected}. Stay calm and let the breath settle.`)
      return
    }
    ...
    const result = resolveShot(
      animal,
      selected,
      82 + (weaponLevel - 1) * 8 - (ownedUpgrades.includes('stock') ? 4 : 0),
    )
  ...
  const handleProcessingComplete = (kept: InventoryItem[]) => {
    setInventory((current) => [...current, ...kept])
    setSupplies((current) => current + kept.length * 12)
    setNotice(`Processing complete. ${kept.length} pieces preserved for the camp.`)
    setPhase('hunt')
  }

  if (phase === 'processing') {
    return <ProcessingScene onFinish={handleProcessingComplete} />
  }

  return (
    <div className="app-shell">
      <header className="topbar">
        <div className="brand-mark">HH<span>3</span></div>
        <div>
          <div className="eyebrow">CURRENT EXPEDITION</div>
          <strong>{phaseLabel}</strong>
        </div>
        <div className="resource-pill">◎ {supplies} supplies</div>
      </header>

      <main className="game-layout">
        <section className="game-column">
          <Scope active={aiming} selected={selected} onFire={handleFire} />
          ...
          <div className="status-row">
            ...
            <button className="secondary-button" onClick={() => setAiming((value) => !value)}>
              {aiming ? 'LOWER SCOPE' : 'READY RIFLE'}
            </button>
          </div>
        ...
        <aside className="side-column">
          ...
          <div className="panel weapon-panel">
            ...
            <div className="stat-row">
              <span>Accuracy</span>
              <strong>{weapon.accuracy}</strong>
            </div>
            <div className="stat-row">
              <span>Level</span>
              <strong>{weapon.level}</strong>
            </div>
            <button className="secondary-button" onClick={() => setWeaponLevel((current) => Math.min(current + 1, weapons.length))}>
              UPGRADE GUN
            </button>
          </div>

          <div className="panel inventory-panel">
            <div className="eyebrow">FIELD CACHE</div>
            <h3>Inventory</h3>
            <div className="inventory-list">
              {inventory.map((item) => (
                <div key={item.id} className="inventory-item">
                  <span className="inventory-color" style={{ background: item.color }} />
          ...
          <div className="panel upgrades-panel">
            ...
            <div className="upgrade-list">
              {upgrades.map((upgrade) => (
                <button key={upgrade.id} className={`upgrade-item ${ownedUpgrades.includes(upgrade.id) ? 'owned' : ''}`} onClick={() => handleUpgrade(upgrade.id)}>
                  <div>
                    <strong>{upgrade.name}</strong>
                    <small>{upgrade.effect}</small>
                  </div>
                  <span>{ownedUpgrades.includes(upgrade.id) ? 'Owned' : `${upgrade.cost}¢`}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <footer>
        <span>WASD / TOUCH TO MOVE</span>
        <span>ACCESSIBILITY: LARGE UI · REDUCED MOTION READY</span>
      </footer>
    </div>
  )
}
